class Solution {
    ...
    isValidSudoku(board) {
        ...
        function isDuplicate(map, key, item){
            if(!map[key]){
                map[key] = {};
            }
            if(map[key][item]){
        ...
        for(let r=0;r<9;r++){
            for(let c=0;c<9;c++){
                ...
                const sqKey = 3*Math.floor(r/3) + Math.floor(c/3);
                console.log({ r, c, sqKey });
                if(item==".") continue;

                if(
                    isDuplicate(rowMap, r, item)
                    || isDuplicate(colMap, c, item)
                    || isDuplicate(sqMap, sqKey, item)
                    ){
                        return false;
                    }
